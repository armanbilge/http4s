...
private[ember] object ChunkedEncoding {
      ...
      nextBytes.flatMap {
        ...
        case Some(bv) =>
          expect match {
            case Left(header) =>
              val nh = header ++ bv
              val endOfHeader = nh.indexOfSlice(backslashrbackslashn)
              if (endOfHeader == 0)
                // strip any leading crlf on header, as this starts with /r/n
                go(expect, nh.drop(backslashrbackslashn.size))
                ...
                val (hdr, rem) = nh.splitAt(endOfHeader + backslashrbackslashn.size)
                readChunkedHeader(hdr.dropRight(backslashrbackslashn.size)) match {
                  case None =>
                    Pull.raiseError[F](
                      EmberException.ChunkedEncodingError(
  ...
  private def parseTrailers[F[_]: MonadThrow](
    ...
    if (buffer.startsWith(Shared.backslashrbackslashn.toArray)) {
      Trailers(Headers.empty, buffer.drop(backslashrbackslashn.size.toInt)).pure[F]
  ...
  private val lastChunk: Chunk[Byte] =
    Chunk.byteVector((ByteVector('0') ++ backslashrbackslashn ++ backslashrbackslashn).compact)
  ...
  def encode[F[_]]: Pipe[F, Byte, Byte] = {
    def encodeChunk(bv: ByteVector): Chunk[Byte] =
      if (bv.isEmpty) Chunk.empty
        ...
        Chunk.byteVector(
          ByteVector.view(
            bv.size.toHexString.toUpperCase.getBytes
          ) ++ backslashrbackslashn ++ bv ++ backslashrbackslashn
        )
    _.mapChunks { ch =>
